...
The app deploys to Render as two services + one managed PostgreSQL database, all defined in `render.yaml` at the repo root.

### Services

| Name | Type | Root Dir | URL |
|---|---|---|---|
| `csvault-api` | Web Service (Node) | `server/` | `https://csvault-api.onrender.com` |
| `csvault-ui` | Static Site | `client/` | `https://csvault-ui.onrender.com` |
| `csvault-db` | PostgreSQL (free) | — | internal connection string |

### How to Deploy (first time)

1. Push this repo to GitHub
2. Go to [render.com](https://render.com) → **New** → **Blueprint**
3. Connect your GitHub repo — Render reads `render.yaml` and creates all three resources automatically
4. In the `csvault-api` service dashboard → **Environment** → set `ANTHROPIC_API_KEY` manually
5. Wait for both services to finish building (~3–5 min)
6. Seed the database once via Render Shell on `csvault-api`:
   ```bash
...
|---|---|---|
| Database | SQLite (`prisma/dev.db`) | PostgreSQL (Render managed) |
| API base URL | `/api` (Vite proxy) | `https://csvault-api.onrender.com/api` (via `VITE_API_URL`) |
| Schema sync | `prisma db push` on start | `prisma db push` runs in `npm start` |
| Frontend routing | Vite dev server | `render.yaml` rewrites `/*` → `/index.html` |

### Environment Variables on Render

**`csvault-api` service:**
- `DATABASE_URL` — auto-injected from `csvault-db`
- `JWT_SECRET` — auto-generated by Render (`generateValue: true`)
- `CLIENT_URL` — `https://csvault-ui.onrender.com` (CORS allowlist)
- `ANTHROPIC_API_KEY` — set manually in dashboard (`sync: false`)
- `NODE_ENV` — `production`

**`csvault-ui` static site:**
- `VITE_API_URL` — `https://csvault-api.onrender.com` (baked into the Vite build)

### Free Tier Caveats

Render free services spin down after 15 minutes of inactivity. The first request after sleep takes ~30 seconds to cold-start. Free PostgreSQL expires after 90 days — upgrade to a paid plan or export data before expiry.
