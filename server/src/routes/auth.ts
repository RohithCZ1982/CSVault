...
authRouter.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    const user = await prisma.user.create({
      data: { name: data.name, email: data.email, passwordHash, level: data.level },
...
authRouter.get('/me', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: req.userId },
      select: { id: true, name: true, email: true, level: true, avatarUrl: true, createdAt: true },
    });
...
authRouter.put('/profile', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const user = await prisma.user.update({
      where: { id: req.userId },
      data: { name, level },
      select: { id: true, name: true, email: true, level: true, avatarUrl: true },
    });
    res.json(user);
  } catch {
    res.status(500).json({ error: 'Server error' });
